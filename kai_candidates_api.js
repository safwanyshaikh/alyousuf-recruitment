...
function testMatchRoute() {
  // Get a real reqId first
  var ss = getMasterSS_();
  var sheet = ss.getSheetByName('_Requirements');
  var firstReqId = sheet.getRange(2, 1).getValue();
  Logger.log('Testing with reqId: ' + firstReqId);
  
  var result = routeMatch_({ reqId: firstReqId, limit: 5 });
  Logger.log(JSON.stringify(result).substring(0, 2000));
}